...
\Titre{Issues related to the behaviour integration step}
\frame{
  \begin{itemize}
...
\begin{frame}{Non-linear case\\\hspace*{1cm}Tensite test}
\begin{textblock}{.30}(0.02,0.17)
\includegraphics[width=2cm]{img/ssna303-mesh.png}
\end{textblock}
\begin{textblock}{.70}(0.27,0.17)
\begin{block}{Tensile test on an elasto-plastic notched beam at finite strain}
~~~~~~
\begin{minipage}{7cm}
\begin{itemize}
...
\begin{frame}{Periodic Representative Volume Element (RVE)\\\hspace*{1cm} Pratical setting}
\begin{block}{Test case setting - elastic model}
\protect\hypertarget{test-case-setting---elastic-model}{}
\begin{itemize}
\item
  Two isotropic materials, each one filling half of a cube
\item
  Aim : compare against \textbf{\texttt{MFEM-MGIS}} results against
...
\begin{block}{Mesh and result}
\protect\hypertarget{mesh-and-result}{}
\vspace*{-5mm}
...
\begin{frame}{Periodic RVE\\\hspace*{1cm} Numerical results on 32 cores}
\begin{block}{Properties}
\begin{itemize}
\item
  14 million unknowns, elastic modelling
\item
  Ref. case on \emph{32 cores} - skylake @ mesocentre Aix-Marseille University
\item
  Main linear solver: Conj. Gradient (iterative), no preconditioner
  used
\end{itemize}
...
\item
 ...
 \begin{itemize}
  \item
    total \textbf{166s}, ~~ matrix assembly 39s, ~~ CGsolver 120s
  \end{itemize}
\item
  MFEM non-linear solve - residual form, a single newton iteration
  \begin{itemize}
  \item
    total \textbf{179s}, ~~ matrix assembly 53s, ~~ CGsolver 117s
  \end{itemize}
\item
  MFEM-MGIS non-linear solve - residual form, a single newton iteration
  \begin{itemize}
  \item
    total \textbf{188s}, ~~ matrix assembly 42s, ~~ CGsolver 133s
  \end{itemize}
\item
...
\begin{frame}{Periodic REV\\\hspace*{1cm}Strong Scaling}
\begin{block}{Strong scaling}
\protect\hypertarget{strong-scaling}{}
\begin{itemize}
\item
...
\item
 ...
 \begin{itemize}
  \item
    \emph{Quite easy to go for a larger nb of cores \(\rightarrow\) need
    a larger machine}
  \end{itemize}
\item
