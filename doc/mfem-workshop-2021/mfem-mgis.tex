...
\Titre{Conclusions and perspectives} \frame{
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      studies and integration in the {\tt PLEIADES} platform.
      \begin{itemize}
      ...
      strain behaviours:
      \begin{itemize}
        \item All behaviours are loaded dynamically at runtime.
      \end{itemize}
      \item Easy extension to other physical phenomea (non linear
      heat-transfer, diffusion, non local mechanics) with basic code
      generation of behaviour integrators (loadable at runtime).
    \end{itemize}
    \item<2-> What is to be improved:
    \begin{itemize}
      \item Overall robustness of the code (critical).
      \item Prediction and handling of the Dirichlet boundary conditions (critical).
    \end{itemize}
    \item<3-> What is to be done:
    \begin{itemize}
      \item Adaptative mesh refinements (requires new data structures in MGIS).
      \item Add new boundary conditions and post-processings
      \item Port to GPUs (requires tremendous work on the MFront and MGIS side).
      \item Support for partial assembly, save/restore, etc...
    \end{itemize}
  \end{itemize}
}
