...
\Titre{MFrontGenericInterfaceSupport \\\hspace*{1cm}to accompany MFront }
\begin{frame}
  \begin{center}
...
\begin{frame}[fragile]{The
  ...
  \begin{itemize}
    ...
    \begin{minipage}{11cm}
    \begin{itemize}
      \item \url{https://github.com/thelfer/mfem-mgis} (core library)\vspace*{-2mm}
      \item \url{https://github.com/latug0/mfem-mgis-examples} (examples)
    \end{itemize}
    \end{minipage}}
  \end{itemize}
\end{frame}
...
\Titre{Issues related to \\\hspace*{1cm}the behaviour integration step} \frame{
  \begin{itemize}
    ...
    solve the linear system computing the Newton correction, it does not
...
\begin{textblock}{.70}(0.2,0.04)
    \includegraphics[width=\textwidth]{img/DirichletBoundaryConditionIssue.pdf}
\end{textblock}
\begin{textblock}{.99}(0.01,0.47)
 \begin{itemize}
    \item Imposing the boundary Dirichlet in {\tt MFEM} leads
    to unphysically strain increments on the element near the boundary at
    the first iteration:
    \begin{itemize}
      \item The problem is critical for very fine 
      meshes.
    ...
    solution based on the tangent problem is provided (through a simpler/intermediate model).
    \begin{itemize}
...
\Titre{Conclusions and perspectives} \frame{
  ...
  \begin{itemize}
    \item What was done ?
    \begin{itemize}
      \item High level declarative API suitable for engineering
      studies\\ and upcoming integration in the {\tt PLEIADES} platform.
      \begin{itemize}
        \item However {\tt MFEM}' API still accessible - low
        level API (not shown here)
      \end{itemize}
      \item Multi-material support
      \item Ability to handle arbitrary complex small and finite
      strain behaviours:
      \begin{itemize}
        \item All behaviours are set at runtime (dynamically loaded libraries).
      \end{itemize}
      \item Basic postprocessing for mechanical modelling (not shown here)
    \end{itemize}
    \medskip
    \item<2-> What comes next ? 
    \begin{itemize}
      \item Overall robustness of the code (critical).
      \item Prediction and handling of the Dirichlet boundary conditions (critical).
      \item Provide more examples (urgent).
      \item Extension to other physical phenomea (non linear
      heat-transfer, diffusion, non local mechanics)  basic code
      generation of behaviour integrators \textit{should be easy}.
      \item Adaptative mesh refinements (requires new data structures in MGIS).
      \item Additionnal boundary conditions and post-processings.
      \item Modeling mechanical systems - contacts and friction (specific problems).
      \item Port to GPUs (requires tremendous work on the MFront and MGIS side).
      \item Support for partial assembly (if possible ...), chekpoint/restart.
    \end{itemize}
  \end{itemize}
  \end{minipage}}
}
