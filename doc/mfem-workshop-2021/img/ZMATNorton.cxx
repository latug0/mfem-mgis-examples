...
ZMATNorton::ZMATNorton() : obp(tfel::material::None) {
...
void ZMATNorton::initialize(ASCII_FILE& file,
                            int dim,
...
void ZMATNorton::initializeMaterialProperties3D(ASCII_FILE& file) {
  using std::find;
  const STRING all_mp_names[4] = {"NortonCoefficient", "NortonExponent",
                                  "PoissonRatio", "YoungModulus"};
  const STRING mp_names3D[4] = {"YoungModulus", "PoissonRatio",
                                "NortonCoefficient", "NortonExponent"};
  for (;;) {
    STRING str = file.getSTRING();
    if (str[0] == '*') {
      file.back();
...
void ZMATNorton::initializeMaterialProperties2D(ASCII_FILE& file) {
  using std::find;
  const STRING all_mp_names[4] = {"NortonCoefficient", "NortonExponent",
                                  "PoissonRatio", "YoungModulus"};
  const STRING mp_names2D[4] = {"YoungModulus", "PoissonRatio",
                                "NortonCoefficient", "NortonExponent"};
  for (;;) {
    STRING str = file.getSTRING();
    if (str[0] == '*') {
...
void ZMATNorton::initializeMaterialProperties1D(ASCII_FILE& file) {
  using std::find;
  const STRING all_mp_names[4] = {"NortonCoefficient", "NortonExponent",
                                  "PoissonRatio", "YoungModulus"};
  const STRING mp_names1D[4] = {"YoungModulus", "PoissonRatio",
                                "NortonCoefficient", "NortonExponent"};
  for (;;) {
    STRING str = file.getSTRING();
    if (str[0] == '*') {
...
void ZMATNorton::initializeParameters3D(ASCII_FILE& file) {
  for (;;) {
    STRING str = file.getSTRING();
    if (str[0] == '*') {
      file.back();
      ...
      tfel::material::NortonParametersInitializer::get()
          .minimal_time_step_scaling_factor = value;
    } else if (str == "maximal_time_step_scaling_factor") {
      const double value = file.getdouble();
      tfel::material::NortonParametersInitializer::get()
          .maximal_time_step_scaling_factor = value;
    } else if (str == "theta") {
      const double value = file.getdouble();
...
void ZMATNorton::initializeParameters2D(ASCII_FILE& file) {
  for (;;) {
    STRING str = file.getSTRING();
    if (str[0] == '*') {
      file.back();
...
void ZMATNorton::initializeParameters1D(ASCII_FILE& file) {
  for (;;) {
    STRING str = file.getSTRING();
    if (str[0] == '*') {
      file.back();
...
INTEGRATION_RESULT* ZMATNorton::integrate(MAT_DATA& mdat,
...
INTEGRATION_RESULT* ZMATNorton::callMFrontBehaviour3D(MAT_DATA& mdat,
                                                      const VECTOR& delta_grad,
                                                      MATRIX*& tg_matrix,
                                                      int flags) {
  typedef tfel::material::Norton<
      tfel::material::ModellingHypothesis::TRIDIMENSIONAL, double, false>
      Norton;
  ...
  Norton b(this->sig, stran, dstran, this->mprops, mdat,
           this->temperature_position, this->evs_positions,
           ZSET::stored_thread_zbase_globals->ptr()->active_clock->get_dtime());
  b.initialize();
...
INTEGRATION_RESULT* ZMATNorton::callMFrontBehaviour2D(MAT_DATA& mdat,
                                                      const VECTOR& delta_grad,
                                                      MATRIX*& tg_matrix,
                                                      int flags) {
  typedef tfel::material::ModellingHypothesis ModellingHypothesis;
  typedef tfel::material::Norton<ModellingHypothesis::GENERALISEDPLANESTRAIN,
                                 double, false>
      Norton;
...
INTEGRATION_RESULT* ZMATNorton::callMFrontBehaviour1D(MAT_DATA& mdat,
                                                      const VECTOR& delta_grad,
                                                      MATRIX*& tg_matrix,
                                                      int flags) {
  typedef tfel::material::ModellingHypothesis ModellingHypothesis;
  typedef tfel::material::Norton<
      ModellingHypothesis::AXISYMMETRICALGENERALISEDPLANESTRAIN, double, false>
      Norton;
  using tfel::math::st2tost2;
  // strain and strain increment
  double stran[3];
...
ZMATNorton::~ZMATNorton() {}  // end of ZMATNorton::~ZMATNorton
