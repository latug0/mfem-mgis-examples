...
int main(int argc, char* argv[]) {
  ...
  // function to check if the solver matches with the preconditioner depending
  // on the test case
  ...
  // some possibilities are removed depending on the refinement and the test
  // case.
  if (match(solver, preconditioner, p.refinement)) {
    kernel(p, use_post_processing, solver, preconditioner, my_info);
  } else
    res = EXIT_FAILURE;
