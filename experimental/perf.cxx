...
int try_several_solvers(TestParameters& p, const bool use_post_processing) {
  ...
  gather_information data;  // gather information (converged/residu/iterations)
                            // for all solver/precond used
  ...
  using std::function;
  typedef function<void(const TestParameters&, const bool, const solver_name,
                        const precond_name, gather_information&)>
      regular_kernel;

  // build your solver / precond list
  // some of them are really slow without a precond such as X+GMRES
  // after a preliminary study, relevent solvers have been selected and stored
  // in the "fast" traversal
  ...
  for (auto solver_iterator : solver_traversal) {
    ...
    for (auto precond_iterator : pc_traversal) {
      if (match(solver_iterator, precond_iterator, p.refinement)) {
        ...
        fun(p, use_post_processing, solver_iterator, precond_iterator, data);
      }
    }
  }
