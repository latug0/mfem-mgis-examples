...
class store_speed_up_values : public std::map<simu_name, list_val> {
 public:
  ...
  void fill(const solver_name a_solv,
  ...
  std::string get_line(const solver_name a_solv,
                       const precond_name a_precond,
                       list_val& a_list) {
    // get the first value
    double coeff = 0.0;
  ...
  std::string get_label(const solver_name a_solv,
                        const precond_name a_precond) {
    std::string label =
        "label=\"" + getName(a_solv) + "-" + getName(a_precond) + "\"";
    return label;
  }

  std::string get_procs(const solver_name a_solv,
                        const precond_name a_precond,
                        list_val& a_list) {
  ...
  std::string get_values(const solver_name a_solv,
                         const precond_name a_precond,
                         list_val a_list) {
    // get the first value
    double coeff = 0.0;
  ...
  std::string get_time_values_only(const solver_name a_solv,
                                   const precond_name a_precond,
                                   list_val a_list) {
    std::string line = "np.array([";
    bool first = true;
...
void build_speed_up_for_python_plot(store_speed_up_values& a_in) {
  ...
  out << "legend_outside = plt.legend(loc=\'center left\',bbox_to_anchor=(1, "
         "0.5))"
      << std::endl;
  out << "namepdf=\'basic-version.pdf\'" << std::endl;
  out << "plt.savefig(namepdf, bbox_extra_artists=(legend_outside,), "
         "bbox_inches=\'tight\')"
      << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  store_speed_up_values storage;  // just a map[sovler,precond]={(proc1,val1),
                                  // .... , (procn,valn)}
  ...
  std::sort(try_classification.begin(), try_classification.end(),
            [](const std::pair<int, info> a, const std::pair<int, info> b) {
              if (a.second.m_time < b.second.m_time)
                return true;
              else
                return false;

              /*			[](const std::pair<int,info> a, const
                 std::pair<int,info> b)
                                      {
                                      if(a.second.m_solver < b.second.m_solver)
                 return true; else if (a.second.m_solver == b.second.m_solver)
                                      {
                                      if(a.second.m_precond <
                 b.second.m_precond) return true; else if(a.second.m_precond ==
                 b.second.m_precond)
                                      {
  ...
  out << "| solver | preconditionner | number of mpi | converged | iterations "
         "| residu | time |"
      << std::endl;
  out << "| "
         ":-------------|:---------------|:---------------:|:---------------:|:"
  ...
  for (auto it : try_classification) {
    ...
    std::string ite =
        inf.m_converged ? std::to_string(inf.m_iteration) : "&cross;";
    ...
    std::string line = "| " + solv + " | " + prec + " | " + proc + " | " +
                       conv + " | " + ite + " | " + res + " | " + time + " | ";
    out << line << std::endl;
  }
