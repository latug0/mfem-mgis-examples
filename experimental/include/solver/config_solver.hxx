...
namespace configuration {
  ...
  static void setLinearSolver(Problem& p,
    ...
    // constexpr mfem_mgis::real relTol	= 1e-12; 		//
    // RelativeTolerance
    ...
    if (!isHypre(a_solver_name) && !isMumps(a_solver_name)) {
      solverParameters.insert(
          mfem_mgis::Parameters{{"AbsoluteTolerance", absTol}});
      solverParameters.insert(
          mfem_mgis::Parameters{{"RelativeTolerance", relTol}});
    }
    ...
    if (isMumps(a_solver_name)) {
      ...
      solverParameters.insert(
          mfem_mgis::Parameters{{"PositiveDefinite", true}});
    }
#endif
    if (a_precond_name != precond_name::ANY) {
      auto precond = buildPrecond(a_precond_name, verbosity);
      solverParameters.insert(
          mfem_mgis::Parameters{{"Preconditioner", precond}});
    }
    ...
    mfem_mgis::Profiler::Utils::Message(
        "INFO: solver: ", current_solver_name,
        " and preconditionner: ", current_precond_name);
