...
namespace fissuration {
  ...
  buildMechanicalProblem(const mfem_mgis::Parameters& common_problem_parameters,
    ...
    std::array<mfem_mgis::real, 3u> p2{0, 0,
                                       mesh_parameters.hp - mesh_parameters.dh};
  ...
  buildMicromorphicProblem(
    ...
    for (const auto& mp : std::map<std::string, double>{
             {"CharacteristicLength", l0}, {"PenalisationFactor", beta}}) {
    ...
    mgis::behaviour::setMaterialProperty(m.s0, "FractureEnergy",
                                         Gc_mp->getValues());
    mgis::behaviour::setMaterialProperty(m.s1, "FractureEnergy",
                                         Gc_mp->getValues());
  ...
  int kernel(const TestParameters& p,
             const bool use_post_processing,
             const solver_name a_solv,
             const precond_name a_precond,
    ...
    auto mechanical_problem =
        buildMechanicalProblem(common_problem_parameters, mesh_parameters);
      ...
      for (mfem_mgis::size_type i = 0; i != n; ++i)
        for (mfem_mgis::size_type i = 0; i != end; ++i) {
          ...
          auto Tg = mfem_mgis::PartialQuadratureFunction::evaluate(
              mechanical_problem->getMaterial("Fuel")
                  .getPartialQuadratureSpacePointer(),
          ...
          mgis::behaviour::setExternalStateVariable(
              mechanical_problem->getMaterial("Fuel").s1, "Temperature",
              Tg->getValues());
          ...
          setLinearSolver(*mechanical_problem, a_solv, a_precond,
                          p.verbosity_level, mfem_mgis::real{1e-8},
                          mfem_mgis::real{1e-6});
          setLinearSolver(*micromorphic_problem, a_solv, a_precond,
                          p.verbosity_level, mfem_mgis::real{0},
                          mfem_mgis::real{1e-6});
          ...
          while (!converged && success != EXIT_FAILURE) {
            ...
            if (iter == 0) {
              mechanical_problem->setSolverParameters(
                  {{"AbsoluteTolerance", 1e-10}, {"RelativeTolerance", reps}});
            ...
            if (!mechanical_output.status) {
              mfem_mgis::Profiler::Utils::Message(
                  "non convergence of the mechanical problem");
            ...
            if (!micromorphic_output.status) {
              mfem_mgis::Profiler::Utils::Message(
                  "non convergence of the micromorphic problem");
              success = EXIT_FAILURE;
              break;
            }
            ...
            if ((iter == iter_max) && (!converged)) {
              mfem_mgis::Profiler::Utils::Message(
                  "non convergence of the fixed-point problem");
              solver_statistics.status = false;
              success = EXIT_FAILURE;
            }
            solver_statistics = micromorphic_output;  // warning; we keep info
                                                      // for the micromorphic pb
          }

          if (use_post_processing)
            common::execute_post_processings(*mechanical_problem, t0, dt);
          if (use_post_processing)
            common::execute_post_processings(*micromorphic_problem, t0, dt);
    ...
    common::fill_statistics(a_info, a_solv, a_precond, solver_statistics,
                            measure);  // res for the micromorphic pb
