 ...
 public:
  ...
  void write() {
    ...
    if (mfem_mgis::Profiler::Utils::is_master()) {
      ...
      for (auto it : m_data) {
        file << it.m_solver << " " << it.m_precond << " " << it.m_converged
             << " " << it.m_iteration << " " << it.m_residu << " " << it.m_time
             << std::endl;
      }
    }
  }
  ...
  void writeMD(std::string a_name, bool add_banner = true) {
    ...
    if (mfem_mgis::Profiler::Utils::is_master()) {
      std::ofstream file(
          a_name, std::ofstream::in | std::ofstream::out | std::ofstream::ate);
      if (add_banner) {
        file << "| solver | preconditionner | converged | iterations | residu "
                "| time |"
             << std::endl;
      }
      ...
      for (auto it : m_data) {
        ...
        std::string ite =
            it.m_converged ? std::to_string(it.m_iteration) : "&cross;";
        ...
        std::string line = "| " + solv + " | " + prec + " | " + conv + " | " +
                           ite + " | " + res + " | " + time + " | ";
        file << line << std::endl;
      }
    }
  ...
  void print() {
    ...
    mfem_mgis::Profiler::Utils::Message("| solver | ", " preconditionner |",
    ...
    for (auto it : m_data) {
      ...
      std::string line = "| " + getName(it.m_solver) + " | " + prec + " | " +
                         conv + " | " + ite + " | " + res + " | " + time +
                         " | ";
