...
namespace common {
  ...
  double solve(Problem& p,
    ...
    if (!statistics.status) {
      mfem_mgis::Profiler::Utils::Message("INFO: ", string_solver, "+",
                                          string_pc, " FAILED");
    }
    return res;
  }
  ...
  void fill_statistics(gather_information& data,
                       Solver solver,
                       Pc pc,
                       Statistics stats,
                       double time) {
    ...
    // fill info data.
    data.add(info{solver, pc, stats.status, stats.iterations,
