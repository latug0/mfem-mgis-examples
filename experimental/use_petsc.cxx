...
void create_petsc_file(const petsc_ksp_type a_solv_name,
                       const petsc_pc_type a_pc_name) {
  ...
  std::string name =
      "petscrc_file_" + getName(a_solv_name) + "_" + getName(a_pc_name);
...
void generate_petsc_files() {
  ...
  for (int ksp_type = 0; ksp_type < petsc_ksp_type_size; ksp_type++) {
    for (int pc_type = 0; pc_type < petsc_pc_type_size; pc_type++) {
      create_petsc_file((petsc_ksp_type)ksp_type, (petsc_pc_type)pc_type);
    }
  }
}
