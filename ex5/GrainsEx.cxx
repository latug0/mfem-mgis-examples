...
bool checkSolution(mfem_mgis::NonLinearEvolutionProblem& problem,
                   const std::size_t i,
                   const mfem_mgis::Parameters& params) {
  const auto b = true;
  if (!b) {
    if (mfem_mgis::getMPIrank() == 0) std::cerr << "Error, check failed\n";
    return false;
  }
  if (mfem_mgis::getMPIrank() == 0) std::cerr << "OK, no error detected\n";
  return true;
}
...
TestParameters parseCommandLineOptions(int& argc, char* argv[]) {
  ...
  args.AddOption(&p.linearsolver, "-ls", "--linearsolver",
                 "identifier of the linear solver: 0 -> Hypre+BoomerAMG "
                 "preconditioning, 1 -> MUMPS");
  args.Parse();
  if (!args.Good()) {
    if (mfem_mgis::getMPIrank() == 0) args.PrintUsage(myout());
    mfem_mgis::finalize();
    exit(0);
  }
  ...
  if (mfem_mgis::getMPIrank() == 0) args.PrintOptions(myout());
...
std::vector<double> findMinPoint(mfem_mgis::FiniteElementDiscretization& fed) {
  const mfem_mgis::FiniteElementSpace<parallel>& fes =
      fed.getFiniteElementSpace<parallel>();
  ...
  for (int i = 0; i < tuples; ++i) {
    ...
    if (coord[0] == minCoord[0] && coord[1] == minCoord[1])
      isInferior = (coord[2] < minCoord[2]);
    if (isInferior) {
      for (int j = 0; j < dim; ++j) {
        minCoord[j] = coord[j];
      }
...
// template <bool parallel>
// void translateMesh(mfem_mgis::FiniteElementDiscretization &fed,
// std::vector<double> vectorTranslate) {
//   const mfem_mgis::FiniteElementSpace<parallel> & fes =
//   fed.getFiniteElementSpace<parallel>(); bool bynodes = fes.GetOrdering() ==
//   mfem::Ordering::byNODES; auto* mesh = fes.GetMesh(); mfem::GridFunction
//   nodes(&(fed.getFiniteElementSpace<parallel>())); mesh->GetNodes(nodes);
//   const size_t nbnodes = static_cast<size_t>(nodes.Size());
//   const size_t dim = static_cast<size_t>(mesh->Dimension()); //fes.GetVDim()
//   std::vector<double> minCoord(dim);
//   const size_t tuples = nbnodes /dim;
...
bool checkNorm(const Orientation3D& ori, const double threshold) {
  ...
  for (uint i = 0; i < 3; i++) {
    if (fabs(sqrt(c[i]) - 1.) > threshold) {
      myout() << std::setprecision(16) << "failure " << i << " " << sqrt(c[i])
              << std::endl;
      return false;
    }
  }
...
bool readOrientations(const TestParameters& p,
  ...
  for (int d = 0; d < cdim; d++) {
    ...
    MFEM_VERIFY(vector_size == v.size(),
                "Vector read is not of expected size, filename: " + filename);
  ...
  for (std::size_t i = 0; i < vector_size; ++i) {
    ...
    MFEM_VERIFY(normalize(data, .5),
                "Orientation vector is wrong, normalization fails");
    MFEM_VERIFY(normalize(data + 3, .5),
                "Orientation vector is wrong, normalization fails");
    // Cross product of X (data) and Y (data+3) vectors that gives Z vector
    // stored at data+6
    crossProduct(data, data + 3, data + 6);
    MFEM_VERIFY(checkNorm(orientations[i], threshold),
...
int executeMFEMMGISTest(const TestParameters& p) {
  ...
  auto fed = std::make_shared<mfem_mgis::FiniteElementDiscretization>(params);
  {
    const auto verbosity =
        mfem_mgis::get_if<int>(params, "GeneralVerbosityLevel", 0);
    // building the non linear problem

    if (verbosity > 1)
      myout() << "Number of processes: " << mfem_mgis::getMPIsize()
              << std::endl;

    std::vector<mfem_mgis::real> minCoord(3);
    minCoord = findMinPoint<p.parallel>(*fed.get());
    if (verbosity > 1)
      myout() << "minCoord " << minCoord[0] << " " << minCoord[1] << " "
              << minCoord[2] << "\n";
    ...
    if (verbosity > 1)
      myout() << "nb grains " << nbgrains << ", nb elements " << nrelem << "\n";
    std::vector<Orientation3D> orientations;
    ...
    for (int i = 1; i <= nbgrains; i++) {
      //      problem.addBehaviourIntegrator("Mechanics", i, p.library,
      //      p.behaviour);
      problem.addBehaviourIntegrator("Mechanics", i, "src/libBehaviour.so",
                                     "Elasticity");
