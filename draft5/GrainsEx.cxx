...
bool checkSolution(mfem_mgis::NonLinearEvolutionProblem& problem,
  ...
  const auto b =
      mfem_mgis::compareToAnalyticalSolution(problem, getSolution(i), params);
  if (!b) {
...
int executeMFEMMGISTest(const TestParameters& p) {
  ...
  auto fed = std::make_shared<mfem_mgis::FiniteElementDiscretization>(params);
  {
    ...
    minCoord = findMinPoint<p.parallel>(*fed.get());
    std::cout << "minCoord " << minCoord[0] << " " << minCoord[1] << " "
              << minCoord[2] << "\n";
    ...
    for (int iel = 0; iel < nrelem; ++iel) {
      ...
      MFEM_VERIFY(attr < nbgrains,
                  "Some elements of the mesh has a number exceeding the number "
                  "of grains");
      ...
      for (int iv = 0; iv < nrvert; ++iv) {
        ...
        for (int d = 0; d < dim; d++) sum_coords[d] += coords[d];
