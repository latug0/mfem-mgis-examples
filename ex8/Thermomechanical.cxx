...
void common_parameters(mfem::OptionsParser &args, TestParameters &p) {
  ...
  args.AddOption(&p.libraryU3SI2, "-lU", "--libraryU3SI2", "Material library for said material.");
  args.AddOption(&p.libraryALFENI, "-lA", "--libraryALFENI", "Material library for said material.");
  args.AddOption(&p.solver_thermo, "-svTh", "--solverTh", "Solver for heat_transfer.");
  args.AddOption(&p.precond_thermo, "-pcTh", "--preconditionnerTh", "Preconditionner for heat transfer.");
  args.AddOption(&p.solver_meca, "-svMc", "--solverMc", "Solver for mechanics.");
  args.AddOption(&p.precond_meca, "-pcMc", "--preconditionnerMc", "Preconditionner for mechanics."); 
...
int main(int argc, char *argv[]) {
  ...
  auto mesh = construct<MeshDiscretization>(ctx,
          ...
          Parameters{
              ...
              {"Materials", Parameters{{"comb", 1}, {"gaine", 2}, {"stiffeners", 3}}},
              {"NumberOfUniformRefinements", p.parallel ? p.refinement : 0},
              {"Parallel", p.parallel}}) | or_die;

  auto heat_transfer_model = make_shared<NonLinearModel>(
          ctx, mesh,
  ...
  auto mechanics_model = make_shared<NonLinearModel>(
          ctx, mesh,
          Parameters{{"FiniteElementFamily", "H1"},
                     {"FiniteElementOrder", p.order},
                     {"Hypothesis", "Tridimensional"},
                     {"UnknownsSize", 3},
                     {"Name", "Mechanics"}}) | or_die;
  ...
  const auto setup = setup_properties(ctx, p, heat_transfer, mechanics, power_history);
  
  apply_boundary_conditions(heat_transfer, mechanics, p, power_history, &u_mech);
