...
void simulation(Problem &problem, double start, double end, double dt, bool pp)
{
	...
	const int nMat = getMaterialsAttributes(*(problem.getFiniteElementDiscretizationPointer())).Max();
	const double young = 222.e9;
	const double poisson = 0.27;
	...
	for (int i = 0; i < nStep; i++)
	{
		...
		{
			problem.setMacroscopicGradientsEvolution([&FXX, &FYY, &FZZ, &SIGXX, &SIGYY, young](const double t) { 
				auto ret = std::vector<mfem_mgis::real>(9, mfem_mgis::real{});
				ret[2] = FZZ;
				ret[1] = FYY - (SIGYY)/young;
				ret[0] = FXX - (SIGXX)/young;
				FXX = ret[0];
				FYY = ret[1];
			...
			auto [tf_integrals, volumes] = computeMeanThermodynamicForcesValues<true>(problem.template getImplementation<true>());
