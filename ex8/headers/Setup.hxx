 ...
 */
inline SetupPropertiesResult
setup_properties(mgis::Context& ctx,
  ...
  mechanics.addBehaviourIntegrator("Mechanics", 1, p.libraryU3SI2, "U3SI2_NortonPRQ"); 
  mechanics.addBehaviourIntegrator("Mechanics", 2, p.libraryALFENI, "ALFENI_IsotropicLinearHardeningPlasticity"); 
  mechanics.addBehaviourIntegrator("Mechanics", 3, p.libraryALFENI, "ALFENI_IsotropicLinearHardeningPlasticity");

  heat_transfer.addBehaviourIntegrator("StationaryNonLinearHeatTransfer", 1, p.libraryU3SI2, "U3SI2_ThermiqueCouplee");
  heat_transfer.addBehaviourIntegrator("StationaryNonLinearHeatTransfer", 2, p.libraryALFENI, "ALFENI_ThermiqueCouplee");
  heat_transfer.addBehaviourIntegrator("StationaryNonLinearHeatTransfer", 3, p.libraryALFENI, "ALFENI_ThermiqueCouplee");
  ...
  for (const int mat_id : {1, 2, 3}) {
    ...
    mgis::behaviour::setExternalStateVariable(m_th.s0, "DeformationGradient", m_mc.s0.gradients, 
      MaterialStateManager::EXTERNAL_STORAGE, MaterialStateManager::NOUPDATE);
    mgis::behaviour::setExternalStateVariable(m_th.s1, "DeformationGradient", m_mc.s1.gradients, 
      MaterialStateManager::EXTERNAL_STORAGE, MaterialStateManager::NOUPDATE);

    if (mat_id == 1) {
      auto sw_model = make_shared<PointWiseModel>(
          ctx, m_mc.getPartialQuadratureSpacePointer(),
          Parameters{{"Library", p.libraryU3SI2}, {"Model", "U3SI2_SolidSwelling"}, {"Hypothesis", "Tridimensional"}}) | or_die;

      auto& m_sw = sw_model->getMaterial();
      const double initial_power = power_history(0.0);
      storage.Pow_s0_sw = std::make_shared<std::vector<mgis::real>>(m_sw.n, initial_power);
      storage.Pow_s1_sw = std::make_shared<std::vector<mgis::real>>(m_sw.n, initial_power);
      ...
      setExternalStateVariable(m_mc.s0, "Swelling", m_sw.s0.internal_state_variables, 
        MaterialStateManager::EXTERNAL_STORAGE, MaterialStateManager::NOUPDATE);
      setExternalStateVariable(m_mc.s1, "Swelling", m_sw.s1.internal_state_variables, 
        MaterialStateManager::EXTERNAL_STORAGE, MaterialStateManager::NOUPDATE);
